...
function AdminProductDetail() {
  ...
  const fetchData = async () => {
    ...
    setCatList(
      resData.categories.map((item: Category) => ({
        value: item.id,
        label: item.name,
      })),
    );
    ...
    setCategories(
      res2.data.data.map((item: Category) => ({
        value: item.id,
        label: item.name,
      })),
    );
      ...
      editInfoForm
        ...
        .then((values: any) => {
          ...
          const newInfo: Info = {
            ...
            categories:
              typeof values.categories[0] === 'object'
  ...
  const handleUpdateProduct = () => {
    ...
    axiosInstance.put(`item/${params.product_id}`, newProduct).then((res) => {
      setCatList(
        categories.filter((cat: any) => {
          ...
          <Box sx={{ flex: '2', display: 'flex', flexDirection: 'column' }}>
                ...
                <Box sx={{ color: 'red', display: 'flex', gap: '5px' }}>
                  {edit && (
                    <>
                      ...
                      />
                      <HighlightOffIcon
                        onClick={() => handleDeleteItem(item.shop.id)}
                        sx={{ cursor: 'pointer' }}
                      />
                    </>
                  )}
            ...
            {edit && (
              <>
                ...
                <Modal
                  title={
                    prices.find((item) => item.shop.id === selected)?.shop.name
                  }
                  open={!!selected}
                  okText="編集"
                  cancelText="キャンセル"
