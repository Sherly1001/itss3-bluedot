                            ...
                            {(fields, { add, remove }) => (
                                <>
                                    {fields.map(({ key, name, ...restField }) => (
                                        <Space key={key} style={{ display: 'flex', marginBottom: 8, justifyContent:'space-between' }}>
                                            <Form.Item
                                                {...restField}
                                                name={[name, 'shop']}
                                                rules={[{ required: true, message: '売り場を入力してください。' }]}
                                            >
                                                <Select allowClear placeholder="売り場" style={{ width: '270px' }}>
                                                    {shops?.map((shop: Shop) => (
                                            ...
                                            <Form.Item
                                                {...restField}
                                                name={[name, 'price']}
                                                rules={[{ required: true, message: '値段を入力してください。' }]}
                                            >
                                                <Input placeholder="値段" type="number" style={{ width: '270px' }} />
                                            </Form.Item>
                                            <MinusCircleOutlined onClick={() => remove(name)} style={{ fontSize: '20px', position: 'relative', top: '-10px' }}/>
                                        </Space>
                                    ))}
                                    <Form.Item>
                                        <Button
